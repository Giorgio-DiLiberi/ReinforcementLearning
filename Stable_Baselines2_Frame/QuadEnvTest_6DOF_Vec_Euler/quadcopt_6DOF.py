...
class QuadcoptEnv_6DOF(gym.Env):
  ...
  def __init__(self, Random_reset = False, Process_perturbations = False):
    ...
    # order is V_N_Err, V_E_Err, V_D_Err, v, p, q, r, Phi, Theta, Psi_error
  ...
  def step(self, action):
      ...
      obs_state = np.concatenate(([VNord_error, VEst_error, VDown_error, self.state[1]], self.state[3:6], [PHI[0], PHI[1], Psi_err])) #, self.state[1] removed visibility over v
  ...
  def reset(self):
      ...
      if self.Random_reset:
        ...
        phi = np_normal(0., 0.008) #[rad]
        theta = np_normal(0., 0.008) #[rad]
        psi = np_normal(140. * 0.0175, 20 * 0.0175) #[rad]
        ...
        self.VNord_ref = np_normal(-1., 1.) #[m/s]
        # if self.VNord_ref<0.:
        #   self.VNord_ref = 0.

        self.VEst_ref = np_normal(-1., 1.) #[m/s]
...
## Conversion from quaternions to Phi_theta
  def quat2Att(self):
      ...
      input: none [Quaternion is extracted from actual self.state]
