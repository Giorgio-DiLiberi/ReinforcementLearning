...
class QuadcoptEnv_6DOF(gym.Env):
  ...
  def __init__(self, Random_reset=False, Process_perturbations=False, Eval_env=False, Lx_i=0.34, Ly_i=0.34, motor_mass_i=0.04, body_mass_i=0.484,
    ...
    self.Lx = Lx_i   #[m] X body Length (squared x configuration)
    self.Ly = Ly_i   #[m] Y body Length
    ...
    # mass of components
    self.motor_mass = motor_mass_i #[kg] mass of one motor+prop
    self.body_mass= body_mass_i #[kg] mass of body frame + electronics (for inertia it is considered as 
    # uniformly distributed in a sphere centered in CG with radius 0.06m)
    self.battery_mass = batt_payload_mass_i #[kg] mass of battery, considered at a distance of 0.06m from CG aligned with it on zb
    ...
    self.nMax_motor = Max_prop_RPM_i / 60 #[RPS]

    # Props Values
    self.D_prop = prop_D_i #[m] diameter for prop
    self.Ct = Prop_Ct_i # Constant of traction tabulated for V=0
    self.Cp = Prop_Cp_i  # Constant of power tabulated for v=0
  ...
  def reset(self):
      ...
      self.Lx = self.Lx_a[self.build_param_count]   #[m] X body Length (squared x configuration)
      self.Ly = self.Ly_a[self.build_param_count]   #[m] Y body Length
      ...
      # mass of components
      self.motor_mass = self.motor_mass_a[self.build_param_count] #[kg] mass of one motor+prop
      self.body_mass= self.body_mass_a[self.build_param_count] #[kg] mass of body frame + electronics (for inertia it is considered as 
      # uniformly distributed in a sphere centered in CG with radius 0.06m)
      self.battery_mass = self.batt_payload_mass_a[self.build_param_count] #[kg] mass of battery, considered at a distance of 0.06m from CG aligned with it on zb
      self.mass = (4*self.motor_mass) + self.body_mass + self.battery_mass  #[kg] total mass of the vehicle
      ...
      # a constant multiplied by the square of prop's rounds per sec:
      # F = Kt * n**2 where n[rounds/s] = Thr * nMax and nMax is evaluated as Kv*nominal_battery_voltage/60
      self.nMax_motor = self.Max_prop_RPM_a[self.build_param_count] / 60 #[RPS]

      # Props Values
      self.D_prop = self.prop_D_a[self.build_param_count] #[m] diameter for prop
      self.Ct = self.Prop_Ct_a[self.build_param_count] # Constant of traction tabulated for V=0
      self.Cp = self.Prop_Cp_a[self.build_param_count] # Constant of power tabulated for v=0
      self.Prop_Kf = self.Ct * self.rho * (self.D_prop**4) #[kg m]==[N/RPS^2]
      self.Prop_Kq = self.Cp * self.rho * (self.D_prop**5)/(2*np.pi) #[kg m^2]
      # now force and torque are evaluated as:
      # F=Kf * N_prop^2 
