...
# This model implements a control only on the torques and receives errors on the quaternion components
# the objective is to achieve attitude control: the reference and errors are evaluated on the 
# componets of the error quaternion: q_err = q_ref * q_mes(compl conj) so the goal is to minimize the 
# relative rotation between desired orientation and actual orientation
import numpy as np
...
class QuadcoptEnv_6DOF(gym.Env):
  ...
  def __init__(self, Random_reset=False, Process_perturbations=False, Lx=0.34, Ly=0.34, motor_mass=0.04, body_mass=0.484,
    ...
    # visibility is given only on p, q, r, q0_err, q1_err, q2_err, q3_err where errors on the 
    ...
    # previous comment
    self.Obs_normalization_vector = np.array([50., 50., 50., 1., 1., 1.]) # normalization constants
    # Random funcs
  ...
  def step(self, action):
      ...
      Err_Quat = self.get_error_quaternion()

      obs_state = np.concatenate((self.state[3:6], Err_Quat[1:4]))
  ...
  def getReward(self):
      ...
      q0_err, q1_err, q2_err, q3_err = self.get_error_quaternion()


      R = (1. * q0_err) - att_weight * (abs(q1_err) + abs(q2_err) + abs(q3_err))\
        - rate_w * (abs(p/50.) + abs(q/50.) + abs(r/50.))
...
## Conversion
  def get_error_quaternion(self):
      ...
      """
      This function, when called, provides the four components of the error quaternion -> q_err = q_ref * q_mes(compl, conj)
      input is taken from class attributes
      output is np.array(error quaternion)
      """
      phi = self.Phi_ref
      theta = self.Theta_ref
      psi = self.Psi_ref

      q1, q2, q3 = -1 * self.state[7:10] # complex conjugate of the measured quaternion
      q0 = self.state[6] 

      q0_ref = cos(phi/2)*cos(theta/2)*cos(psi/2) + sin(phi/2)*sin(theta/2)*sin(psi/2)
      q1_ref = sin(phi/2)*cos(theta/2)*cos(psi/2) - cos(phi/2)*sin(theta/2)*sin(psi/2)
      q2_ref = cos(phi/2)*sin(theta/2)*cos(psi/2) + sin(phi/2)*cos(theta/2)*sin(psi/2)
      q3_ref = cos(phi/2)*cos(theta/2)*sin(psi/2) - sin(phi/2)*sin(theta/2)*cos(psi/2)

      q0_err = q0_ref * q0 - q1_ref * q1 - q2_ref * q2 - q3_ref * q3
      q1_err = q0_ref * q1 + q1_ref * q0 + q2_ref * q3 - q3_ref * q2
      q2_err = q0_ref * q2 - q1_ref * q3 + q2_ref * q0 + q3_ref * q1
      q3_err = q0_ref * q3 + q1_ref * q2 - q2_ref * q1 + q3_ref * q0

      if q0_err < 0:
        q3_err = -q3_err

      return np.array([q0_err, q1_err, q2_err, q3_err])
