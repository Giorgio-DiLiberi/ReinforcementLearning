...
# takes as input average thr, and torques along xb, yb, zb and outputs torques requests, they 
...
class QuadcoptEnvV5(gym.Env):
  """Quadcopter Environment that follows gym interface"""
  metadata = {'render.modes': ['human']}

  def __init__(self):
    super(QuadcoptEnvV5, self).__init__()


    # Define action and observation space
    # They must be gym.spaces objects
    # Definition of action space with 4 control actions representing Throttle, xb, yb, zb torques.
    # As for the airplane, those torques are considered immediately applied without the need for a 
    # PID controller to run motors according to references  
    ...
    self.d2 = 1 - self.dTt - 0.1 # Assumed value for first constant in action to throttle mapping
  ...
  def step(self, action):
      ...
      Throttles = self.getThrsFromControls(action) # commands are the actions given by the policy
