...
# This code implemets the Humming_env for position control with different build parameters to
# prove that the polict can control the quad even with different build parameters
import numpy as np
...
class Hummingbird_6DOF(gym.Env):
  ...
  def __init__(self, Random_reset = False, Process_perturbations = False):
    ...
    # plus configuration
    self.Lx = 0.75   #[m] X body Length (squared x configuration)
    self.Ly = 0.75   #[m] Y body Length
    ...
    # mass of components
    self.mass = 1.9   #[kg] mass is 1900 grams for 15 inches quad
    self.motor_mass = 0.20 #[kg] mass of one motor+prop
    self.body_mass= 0.700 #[kg] mass of body frame + electronics (for inertia it is considered as 
    # uniformly distributed in a sphere centered in CG with radius 0.06m)
    self.battery_mass = 0.400 #[kg] mass of battery, considered at a distance of 0.06m from CG aligned with it on zb
    ...
    self.nMax_motor = 6500 / 60 #[RPS] set from the thesis

    # Props Values
    self.D_prop = 0.381 #[m] diameter for 7 inch prop
    self.prop_mean_chord = 0.020 #[m] 1.5 cm as mean chord for prop profile
    self.prop_Theta0 = 0.21 #[rad] = atan(pitch / (2*pi*0.75*Rprop))
    self.Ct = 0.085 # Constant of traction tabulated for V=0
    self.Cp = 0.0377  # Constant of power tabulated for v=0
