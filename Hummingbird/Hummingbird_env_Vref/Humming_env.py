...
class Hummingbird_6DOF(gym.Env):
  ...
  def __init__(self, Random_reset = False, Process_perturbations = False, Position_reference = True):
    ...
    # Wind 
    self.WindVel = np.array([0., 0., 0.]) # wind velocity in NED frame->to be subtacted to the Vb before linear acceleration computation
    self.Gusts = 0.1 # value for variance of gusts added as normal disturbances to wind velocity7
    # wind model is added only to the equation which evaluates the forces generated by engines due to the fact
    # that relative air speed influences only the External actions generation
...
## In this sections are defined functions to evaluate forces and derivatives to make the step function easy to read
  ...
  def eqnsOfMotion(self, State, Throttles):
      ...
      if self.Process_perturbations:
        ...
        Vw_NED = self.WindVel # total wind in NED frame as stable wind plus gusts
      ...
      # wind projection
      Vw_body = np.dot(LBE, Vw_NED) # projection of NED wind velocity to Body frame
      
      Vb_rel = Vb - Vw_body # relative airspeed to bodiframe
      ...
      Vm1 = Vb_rel + np.cross(Omega, self.rM1)
      Vm2 = Vb_rel + np.cross(Omega, self.rM2)
      Vm3 = Vb_rel + np.cross(Omega, self.rM3)
      Vm4 = Vb_rel + np.cross(Omega, self.rM4)
