...
# in terms of Phi_ref, Theta_ref, r_ref
import numpy as np
...
## In this sections are defined functions to evaluate forces and derivatives to make the step function easy to read
  ...
  def Motor(self, Throttle, Vm):
      ...
      #Delta_Thrust = self.rho * np.pi * self.prop_mean_chord * (np.sqrt(self.dTt) * self.nMax_motor * 2 * np.pi) * ((self.D_prop**2)/4) * (- vi + wm + self.vh)
