...
if __name__ == '__main__':
    ...
    env = SubprocVecEnv([lambda : Hummingbird_3DOF(Random_reset=True, Process_perturbations=True) for num in range(cpu)], start_method='spawn')

    ### AGENT MODEL AND CALLBACK DEFINITION

    eval_env = DummyVecEnv([lambda : Hummingbird_3DOF(Random_reset=False, Process_perturbations=False)]) # Definition of one evaluation environment
    ...
    model = PPO2(MlpPolicy, env, verbose=1, learning_rate=LearningRate, ent_coef=5e-8, lam=0.99,
            ...
            noptepochs=128, n_steps=8156, n_cpu_tf_sess=4)
    ...
    for i in range(1, 100): ## policies name format "PPO_Quad_<numberOfAttempt>.zip"
        ...
        filename_check = "/home/ghost/giorgio_diliberi/ReinforcementLearning/Hummingbird/Hummingbird_PhiThetaRref/Policies/PPO_Quad_" + str(i) + ".zip"
