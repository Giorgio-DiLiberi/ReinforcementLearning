...
# This code implement a MonteCarlo simulation of N trials
import os
...
if Policy_loading_mode == "last":
  ...
  Policy2Load = "EvalClbkLogs/best_model9.zip" # last best policy name
...
for count1 in range(N_trials): # for 25 simulations
  ...
  X_Err = env.state[10] - env.X_ref
  Y_Err = env.state[11] - env.Y_ref
  Z_Err = env.state[12] - env.Z_ref

  dist = np.sqrt((X_Err**2) + (Y_Err**2) + (Z_Err**2))
  dist_mem = [dist]
  ...
  final_distance = np.sqrt((X_Err**2) + (Y_Err**2) + (Z_Err**2))
