...
for i in range(tieme_steps_to_simulate): #last number is excluded
    ...
    # if i>=256 and i<3072 and i%32==0:
    #   angle = 0.1 * (env.elapsed_time_steps-256) * 0.04
...
plt.savefig('SimulationResults/Phi_Theta.jpg')
...
plt.ylabel('Psi [deg]')
plt.title('Psi')
plt.savefig('SimulationResults/Psi.jpg')
