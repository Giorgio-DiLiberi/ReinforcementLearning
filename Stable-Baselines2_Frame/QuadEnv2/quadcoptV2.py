...
class QuadcoptEnvV2(gym.Env):
  ...
  def __init__(self):
    ...
    self.Kt = 0.1 ##[m] ATTENTION, proportional constant for assumption about the torque of the motor
    ...
    self.SnX = 0.02 #[m^2] cross sections in m^2 referred to the Cd
    self.SnY = 0.02 #[m^2]
    self.SnZ = 0.05 #[m^2] Z normal surface is bigger than other ones    
...
## In this sections are defined functions to evaluate forces and derivatives to make the step function easy to read
  ...
  def Drag(self, V, Sn, Cd):
      ...
      drag = - 0.5 * self.rho * V * abs(V) * Sn * Cd  #[N]
  ...
  def eqnsOfMotion(self, State, Throttle):
      ...
      u, v, w, p, q, r, q0, q1, q2, q3 = State[0:10] # measure units are reported in a comment in __init__()
      dT1, dT2, dT3, dT4 = Throttle
      

      #THRUST Evaluation [N]
      # is evaluated positive for thrust oriented in the negative verse of Zb
      # according to how props generate the thrust, signs have to be adjusted in the equations
      # keeping in ming that those vectors are cosidered  
      ...
      # Evaluation of LINEAR ACCELERATION components [m/s^2]: first total forces are calculated, than divided by mass
