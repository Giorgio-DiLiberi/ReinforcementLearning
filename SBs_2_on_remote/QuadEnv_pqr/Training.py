...
if __name__ == '__main__':
    ...
    env = SubprocVecEnv([lambda : QuadcoptEnv_pqr(Random_reset=False, Process_perturbations=False) for num in range(cpu)], start_method='spawn')

    ### AGENT MODEL AND CALLBACK DEFINITION

    eval_env = DummyVecEnv([lambda : QuadcoptEnv_pqr(Random_reset=True, Process_perturbations=False)]) # Definition of one evaluation environment
    ...
    for i in range(1, 100): ## policies name format "PPO_Quad_<numberOfAttempt>.zip"
        ...
        filename_check = "/home/ghost/giorgio_diliberi/ReinforcementLearning/SBs_2_on_remote/QuadEnv_pqr/Policies/PPO_Quad_" + str(i) + ".zip"
