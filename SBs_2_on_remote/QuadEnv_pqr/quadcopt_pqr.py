...
# this model include a mathematical rapresentation of the quadcopter in which the controls from the policy
# are the pseudo commands wich then are translated on throttles for the single motors by a mixer function
# the observation space is made of three errors on the three angular boby velocities calculated from a refernce 
# value for those components
import numpy as np
...
class QuadcoptEnv_pqr(gym.Env):
  """Quadcopter Environment that follows gym interface"""
  metadata = {'render.modes': ['human']}

  def __init__(self, Random_reset = False, Process_perturbations = False):
    super(QuadcoptEnv_pqr, self).__init__()


    # Define action and observation space
    # They must be gym.spaces objects
    # Definition of action space with control over the pseud commands
    # dA, dE, and dR while throttle value is given manually
    ...
    # p[-50,50][rad/s], q[-50,50][rad/s], r[-50,50][rad/s]
    ...
    # the policy is given visibility only on p, q, r errors
    highObsSpace = np.array([1.2, 1.2, 1.2])
    ...
    # previous comment
    self.Obs_normalization_vector = np.array([20., 20., 20.]) # normalization constants
    # Random funcs
    ...
    ## External throttle value (fixed)
    self.AvgThr_given = 0.01
    ...
    self.p_ref = 1.75 #[rad/2]
    self.q_ref = 0. #[rad/2] 
    self.r_ref = 0. #[rad/2] references for angular velocities error is evaluated in step func
  ...
  def step(self, action):
      ...
      controls = np.array([self.AvgThr_given, action[0], 0., 0.]) ## This variable is used to make possible the separation of actions 
      # in this example actions represent pseudo controls and avg throttle is given
      ...
      p_error = self.p_ref - self.state[3]
      q_error = self.q_ref - self.state[4]
      r_error = self.r_ref - self.state[5]
      obs_state = np.array([p_error, q_error, r_error])
  ...
  def reset(self):
      ...
      if self.Random_reset:
        ...
        p_reset = np_normal(0., 5*1.75)
        q_reset = np_normal(0., 5*1.75)
        r_reset = np_normal(0., 5*1.75)
  ...
  def getReward(self):
      ...
      R = 1. - (pq_weight * (abs(p_error)/20.)) - (pq_weight * (abs(q_error)/20.)) - (r_weight * (abs(r_error)/20.))
